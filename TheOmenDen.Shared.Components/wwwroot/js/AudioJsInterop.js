import { Howl, Howler } from '../wwwroot/js/howler';
...
window.howl = {
    play: function (dotnetReference, options) {
        ...
        window.soundId = howl.play();

        window.howlInstances[soundId] = {
            howl,
            options
        };
    ...
    unload: function (id) {
        ...
        if (howl) {
            ...
            window.howlInstances[id] = null;
            delete window.instances[id];
        }
    },
    ...
    destroy: function () {
        Object.keys(window.howlInstances).forEach(key => {
            try {
                unload(key);
            } catch(exception) {
                // no-op
...
window.howler = {
    ...
    isCodecSupported: function (extension) {
        return extension ? Howler._codecs[extensions.replace(/^x-/, '')] : false;
    }
};
